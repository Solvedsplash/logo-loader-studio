...
/**
 * Deterministically generates a Lottie JSON string from the stateless core
 * engine by sampling it once per frame.
 *
 * Note this bakes only the logo layer's transform track. Path-drawing strokes and
 * decorations (rings, orbits, sweeps) are canvas-level effects with no Lottie
 * shape equivalent, so path-family presets export as the logo's fade/scale
 * envelope alone — use GIF or WebM to keep the stroke animation.
 *
 * @param {object} animation Resolved animation object
 * @param {string} svgBase64 base64 data URI of the logo SVG
 * @param {Array} svgPathsData Path measurements
 * @param {number} fps Target frames per second
 * @returns {string} Lottie JSON
 */
export function generateLottieJson(animation, svgBase64, svgPathsData = [], fps = 60) {
...
export default { generateLottieJson };
