...
export function renderFrame(ctx, progress, logoImg, animation, svgPaths = []) {
  ...
  // CRITICAL: Always reset filter at the top of every frame to prevent contamination between frames
  ctx.filter = 'none';
  ...
  switch (FAMILY) {
    ...
    case 'pro-morph': {
      // Professional Shape Morph (v3.0)
      // 0.0 - 0.35: Morph Logo into Ring
      // 0.35 - 0.65: Professional Hold / Circular Pulse
      // 0.65 - 1.0: Morph Ring back into Logo
      
      const morphT = progress; 
      
      if (morphT < 0.35) {
        const r = morphT / 0.35;
        scale = lerp(1, 0.45, EASE.easeInOut(r));
        blur = lerp(0, 24, EASE.easeInOut(r));
        opacity = lerp(1, 0, EASE.easeInOut(r * 1.5)); // Fade logo out faster
      } else if (morphT > 0.65) {
        const r = (morphT - 0.65) / 0.35;
        scale = lerp(0.45, 1, EASE.backOut(r));
        blur = lerp(24, 0, EASE.easeInOut(r));
        opacity = lerp(0, 1, EASE.easeInOut(Math.max(0, (r - 0.2) / 0.8))); // Fade logo in
      } else {
        scale = 0.45;
        blur = 24;
        opacity = 0;
  ...
  // CRITICAL: Reset filter after every logo draw — prevents contamination of particle/ring/path overlays
  ctx.filter = 'none';
  ...
  if (FAMILY === 'pro-morph') {
    // CRITICAL: Explicitly reset filter before drawing ring — must never inherit logo blur
    ctx.filter = 'none';

    const ringStart = 0.05;
    const ringEnd = 0.95;
    ...
    if (progress > ringStart && progress < ringEnd) {
      ...
      if (progress < 0.35) {
        const r = (progress - ringStart) / (0.35 - ringStart);
        ringAlpha = lerp(0, 1, EASE.easeInOut(r));
        ringScale = lerp(0.8, 1, EASE.easeInOut(r));
      } else if (progress > 0.65) {
        const r = (progress - 0.65) / (ringEnd - 0.65);
        ringAlpha = lerp(1, 0, EASE.easeInOut(r));
        ringScale = lerp(1, 1.2, EASE.easeInOut(r));
      } else {
        const pr = (progress - 0.35) / 0.3;
        ringScale = 1 + Math.sin(pr * Math.PI * 4) * 0.04;
        ringRotate = pr * Math.PI * 2;
      }
      ...
      if (ringAlpha > 0) {
        ...
        ctx.stroke();
        ctx.restore();
        ctx.filter = 'none'; // Reset after restore in case of stale state
      }
    }
  }
}
