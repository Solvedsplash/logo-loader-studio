...
async function getFFmpeg() {
  if (!ffmpegPromise) {
    ffmpegPromise = (async () => {
      // Note: on failure this promise is cleared below rather than cached, so a
      // transient CDN outage doesn't permanently break exports for the tab.
    ...
    // Caching the *promise* means a single failed load would be replayed forever.
    // Drop it on rejection so the next attempt genuinely retries.
    ffmpegPromise.catch(() => { ffmpegPromise = null; });
  }
  return ffmpegPromise;
}

// ffmpeg.wasm is a single shared virtual filesystem, and both exports would write
// the same frames.raw / out.webm. Serialise calls instead of letting a second
// export corrupt the first.
 ...
 */
export function exportToFfmpegWebM(opts) {
  return serialize(() => runFfmpegWebMExport(opts));
}

async function runFfmpegWebMExport({
  CoreEngine,
  animation,
  logoImg,
  svgPathsData = [],
  ...
  for (let i = 0; i < totalFrames; i++) {
    // Sample over [0, 1) so the last frame isn't a duplicate of frame 0.
    const progress = i / totalFrames;

    // This path exists to produce a real alpha channel, so the background is
    // forced transparent regardless of what the animation carries.
