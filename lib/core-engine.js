...
(function (global) {
  ...
  function applyPathDraw(state, t, raw, P, svgPaths, animation) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      var len = Math.max(1, pd.length || 1000);
      // Add a small padding beyond the path length so the dash never clips
      // at the very end of arc-heavy paths (rounded rects, circles, etc.).
      var lenPad = len + 5;
      ...
      if (mode === 'comet') {
        ...
        dash = [tail, lenPad];
        dashOffset = -dp * (lenPad + tail) + tail;
      } else if (mode === 'march') {
